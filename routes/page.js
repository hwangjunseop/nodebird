...
// 모든 요청에 대해 콩통적으로 수행할 작업을 정의하는 미들웨어 추가
router.use((req, res, next) => {
    res.locals.user = null; // 사용자 정보 초기화
    res.locals.followerCount = 0; // 팔로워 수 초기화
    res.locals.followingCount = 0; // 팔로잉 수 초기화
    res.locals.followingIdList = []; // 팔로잉 ID리스트롤 초기화
    next(); // 다음 미들웨어 또는 라우터로 제어를 전달
});

// '/profile' 경로로 GET 요청이 들어오면 renderProfile 컨드롤러 실행
router.get('/profile', renderProfile);
// '/join' 경로로 GET요청이 들어오면 renderJoin 컨트롤러 실행
router.get('/join', renderJoin);
// 루트경로 ('/')로 GET요청이 들어오면 renderMain 컨트롤러를 실행
